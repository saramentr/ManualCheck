...
# prjkey_:_urlPort_:_prjlgn_:_organ_:_repo
    for i in listWorkLine:
      ...
      if not os.path.exists(prePathWorkGit+"/"+sepListTmp[3]+"_"+sepListTmp[4]):
        ...
        continue
      os.system("echo $PATH")
      os.system("export PATH=~/sonar-scanner-4.6.1.2450-linux/bin/:/usr/lib/jvm/java-11-openjdk-amd64/bin/:$PATH&&sonar-scanner -Dsonar.projectKey="+sepListTmp[0]+" -Dsonar.sources="+prePathWorkGit+"/"+sepListTmp[3]+"_"+sepListTmp[4]+" -Dsonar.host.url="+sepListTmp[1] +" -Dsonar.login="+sepListTmp[2])
# fix time to 1000  
  time.sleep(100)
  os.system("df -h")
